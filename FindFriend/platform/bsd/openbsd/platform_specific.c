...
FFResult platform_init(void) {
    g_initialized = true;
    FF_INFO("OpenBSD platform initialized");
    return FF_OK;
}
...
void platform_cleanup(void) {
    if (g_initialized) {
        FF_INFO("OpenBSD platform cleanup");
        g_initialized = false;
    }
}
...
FFResult platform_get_system_info(char* buffer, size_t buffer_size) {
    ...
    if (sysctlbyname("kern.osrelease", &version, &len, NULL, 0) == 0) {
        snprintf(buffer + strlen(buffer), buffer_size - strlen(buffer), "OS Version: OpenBSD %d\n", version[0]);
    }
