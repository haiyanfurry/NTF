...
DBResult db_login_user(const char *username, const char *password, User *user) {
    ...
    sprintf(sql, "UPDATE users SET online = 1, last_seen = %lld WHERE id = %d;", (long long)now, user->id);
...
DBResult db_update_user(const User *user) {
    char sql[1024];
    snprintf(sql, sizeof(sql), "UPDATE users SET nickname = '%s', avatar_hash = '%s', signature = '%s', "
            "identity = %d, tags = %d, latitude = %f, longitude = %f, location_enabled = %d, "
            "last_seen = %lld, online = %d, exhibition_name = '%s', exhibition_url = '%s', "
            ...
            (long long)user->last_seen, user->online, user->exhibition_name, user->exhibition_url,
