...
int64_t platform_get_free_memory(void) {
    ...
    if (sysctlbyname("vm.stats.vm.v_page_size", &pagesize, &len, NULL, 0) != 0) {
        return 0;
    }

    len = sizeof(freecount);
    if (sysctlbyname("vm.stats.vm.v_free_count", &freecount, &len, NULL, 0) != 0) {
        return 0;
    }

    return (int64_t)(pagesize * freecount);
#else
    return 0;
#endif
}
