...
FindFriend 现在包含一个完整的跨平台核心接口抽象层和调试系统，支持 Linux、Windows、macOS、iOS 和 Android。
...
│   └── utils/                # 工具函数
├── platform_linux_redhat/    # Linux 平台实现
├── platform_linux_debian/    # Linux Debian 平台实现
├── platform_windows/         # Windows 平台实现
...
在 Linux 上，已编译好的 CLI 工具位于 `./build_test/bin/FindFriendCLI`。
...
在 Windows 平台上提供了完整的 GUI 调试界面，通过 `platform_windows/win32_debug.cpp` 实现，包含：
- 用户登录/登出
- 消息发送
- 位置上报
- 调试日志查看
...
所有其他平台都通过 `core_interface.h` 提供的统一接口调用核心逻辑，只需实现平台特定的 UI 层。
